...
static uint32_t get_u32(const struct gguf_context * gctx, const char * key) {
    ...
    if (id < 0) DIFFUSE_DIE("missing GGUF key: %s", key);
...
static uint32_t get_u32_multi(const struct gguf_context * gctx,
                               const char * suffix, uint32_t def = 0, bool required = true) {
    ...
    for (int i = 0; prefixes[i]; i++) {
        ...
        if (id >= 0) return gguf_get_val_u32(gctx, id);
    }
    if (!required) return def;
    DIFFUSE_DIE("missing GGUF key: *.%s", suffix);
    return 0;
}
...
static float get_f32_multi(const struct gguf_context * gctx,
    ...
    for (int i = 0; prefixes[i]; i++) {
        snprintf(buf, sizeof(buf), "%s.%s", prefixes[i], suffix);
        int64_t id = gguf_find_key(gctx, buf);
        if (id >= 0) return gguf_get_val_f32(gctx, id);
    }
    return def;
}
...
static std::string detect_arch_prefix(const struct gguf_context * gctx) {
    ...
    if (id >= 0) return gguf_get_val_str(gctx, id);
...
static struct ggml_tensor * get_tensor(struct ggml_context * ctx, const char * name) {
    ...
    if (!t) DIFFUSE_DIE("tensor not found: %s", name);
    return t;
}

static struct ggml_tensor * try_tensor(struct ggml_context * ctx, const char * name) {
...
diffuse_model * diffuse_model_load_impl(const std::string & path, int n_threads) {
    ...
    struct gguf_init_params gparams = { false, &meta_ctx };
    struct gguf_context * gctx = gguf_init_from_file(path.c_str(), gparams);
    if (!gctx) DIFFUSE_DIE("failed to open GGUF: %s", path.c_str());

    auto * model = new diffuse_model();
    auto & hp = model->hparams;

    std::string arch_prefix = detect_arch_prefix(gctx);
    DIFFUSE_LOG("  arch prefix: %s", arch_prefix.c_str());
    ...
    hp.n_vocab       = get_u32_multi(gctx, "vocab_size", 0, false);
    hp.mask_token_id = get_u32_multi(gctx, "mask_token_id", 0, false);
    hp.eos_token_id  = get_u32_multi(gctx, "eos_token_id", 0, false);
    ...
    hp.block_length  = get_u32_multi(gctx, "block_length", 0, false);
    ...
    // Read model type
    {
        ...
        } else if (arch_prefix == "qwen2") {
            model->model_type = "dream";
        } else if (arch_prefix == "llama") {
            model->model_type = "llada";
        } else {
    ...
    if (model->model_type == "llada2_moe") {
        model->arch = diffuse_arch::LLaDA2_MoE;
    } else if (model->model_type == "dream") {
        model->arch = diffuse_arch::DREAM;
    } else {
        model->arch = diffuse_arch::LLaDA;
    }

    const char * arch_name =
        model->arch == diffuse_arch::LLaDA2_MoE ? "LLaDA2 MoE (block diffusion)" :
        model->arch == diffuse_arch::DREAM      ? "Dream (flat diffusion, Qwen2.5)" :
                                                   "LLaDA (flat diffusion)";
    ...
    DIFFUSE_LOG("  rope_theta=%.0f, rms_eps=%.1e, mask=%u, eos=%u, block_len=%u",
                hp.rope_theta, hp.rms_norm_eps, hp.mask_token_id, hp.eos_token_id, hp.block_length);
    ...
    if (hp.n_vocab == 0) {
        ...
        DIFFUSE_LOG("  n_vocab inferred: %u", hp.n_vocab);
    }

    struct ggml_tensor * lm_head = try_tensor(meta_ctx, "output.weight");
    model->output = lm_head ? lm_head : model->tok_embd;

    // ── Load layers ────────────────────────────────────────────
    if (model->arch == diffuse_arch::LLaDA2_MoE) {
        // MoE layers
        ...
        DIFFUSE_LOG("  MoE: %u experts (top-%u), %u shared, moe_ff=%u, first_dense=%u",
                    ...
                    model->n_shared_experts, model->moe_intermediate, model->first_k_dense);
        DIFFUSE_LOG("  head_dim=%u, rotary_dim=%u, qk_norm=%s, routed_scale=%.1f",
                    model->head_dim, model->rotary_dim,
                    model->use_qk_norm ? "yes" : "no", model->routed_scaling);
        ...
        for (uint32_t i = 0; i < hp.n_layer; i++) {
            ...
            if (!ml.is_moe) {
                ...
                ml.gate_bias   = try_tensor(meta_ctx, fmt_layer("blk.%d.moe_gate_bias.weight", i).c_str());
        ...
        for (uint32_t i = 0; i < hp.n_layer; i++) {
            ...
            // Optional QKV biases (Dream/Qwen2.5)
            l.bq = try_tensor(meta_ctx, fmt_layer("blk.%d.attn_q.bias", i).c_str());
            l.bk = try_tensor(meta_ctx, fmt_layer("blk.%d.attn_k.bias", i).c_str());
            l.bv = try_tensor(meta_ctx, fmt_layer("blk.%d.attn_v.bias", i).c_str());
        }
    }

    DIFFUSE_LOG("model loaded: %lld tensors", (long long)gguf_get_n_tensors(gctx));
    gguf_free(gctx);
    return model;
}
